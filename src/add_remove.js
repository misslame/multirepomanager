...
try {
    ...
    if (args.length == MAX_ARG_LENGTH) {
        ...
        throw new Error(
            '[ERROR]: InvalidArgumentsError: Please provide repo owner and repo name \n.Ex. npm run add -- owner/reponame',
        );
    }
    ...
    if (command === 'add') {
        ...
        throw new Error(
            '[ERROR]: Command not given for action to be performed. This is probably something wrong with the tool.',
        );
    }
} catch (exception) {
    if (exception instanceof RequestError) {
        console.log(
            '[ERROR]: RequestError: Could not determine if repository exists, please provide a valid repository.',
        );
